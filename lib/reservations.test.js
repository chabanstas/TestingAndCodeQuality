...
describe('fetch', () => {
  ...
  it('should be mocked and not create a database record', () => {
    expect(reservation.fetch()).toBeUndefined();
  });
});
...
describe('save', () => {
  ...
  it.skip('should resolve with the id upon success', async () => {
    const value = { foo: `bar` };
    const expected = [1];
...
describe('validate', () => {
  ...
  it('should be called and reject empty input', async () => {
    ...
    await expect(reservations.validate(value)).rejects.toThrow('Cannot read properties of undefined (reading \'validate\')');
...
describe('create', () => {
  ...
  it.skip('should create a reservation if there are no validate problems', async () => {
    // Prepare to require
    const expectedInsetId = 1;

    const mockInsert = jest.fn().mockResolvedValue([expectedInsetId]);
    ...
    // Execute and check
    await expect(reservations.create(reservation)).resolves.toStrictEqual(expectedInsetId);
